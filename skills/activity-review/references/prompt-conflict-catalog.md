...
| 最终活动 Skill 链接 `<builder-root>/policies/...` 或硬编码 repo-local Builder 路径 | Builder 不在活动 Docker 的 `/activity` 挂载里，Runtime Agent 无法读取 | 把必要护栏写成活动本地指令；开发期参考不进入已上传 Skill |
| scaffold 仍有 `TODO_ACTIVITY_AUTHOR`、`<id>` 或“写完后替换”却继续验收/同步 | 模板示例会成为 Runtime Agent 的真实指令 | 完成 authoring residue 清理；让 verifier 阻断任何残留 |
