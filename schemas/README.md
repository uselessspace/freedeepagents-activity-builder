...
- **创作期**（你本地）：`<package>/schemas/*` + `activity_verifier.py` 校验你写的文件，随插件版本走（当前 `Bundle version: 0.4.17`，见顶部）。
