...
This activity runs in **card-system mode** (the runtime's only mode) with the data mode declared in `runtime.json`.

- Managed SQLite is the default for new activities. Access it only inside Activity-owned domain tools/handlers through `ctx.database`; never expose SQL or a database path to the Agent.
- If this activity is classified typed-KV or hybrid, small Prompt-aware state lives in `/instance/data.json`, declared by `data.schema.json`, and is mutated through activity @tools or `data_*`.
...
- If a tool call returned an error or you emitted the wrong card, call `card_retract(card_id)` before re-emitting (same-turn only). Business data mistakes are fixed by re-calling the corresponding domain @tool with corrected args.
...
- The runtime injects a generic smalltalk fallback. The host Skill is the single routing authority for fast paths; this entrypoint and the cards Skill do not duplicate greeting or business conditions.
