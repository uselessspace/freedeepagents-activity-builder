...
> - 🟡 **verifier 强制**：违反 → `python3 <package>/tools/activity_verifier.py <project-root>` 报 ERROR（exit 1），目录验收前必修；运行时不一定立刻炸。`<package>` 必须是当前实际加载的 Builder 根目录，不要用 `grep` 过滤 verifier 输出。
