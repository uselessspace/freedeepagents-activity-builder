...
## Hard checks (ERROR, block directory verification)
...
| 4 | **typed KV data store wiring** | `runtime.data_schema_enabled: true` requires `data.schema.json` to exist | add `data.schema.json`, or flip the flag back to false |
| 4a | **managed database declaration** | enabled database omits engine/scope/access or uses unknown values | declare SQLite plus explicit instance/activity_type scope and Agent/user access |
...
| 10 | Generic-runtime boundary | activity-id literal in `app/`, `schemas/` | move logic to `activities/<id>/skills/` |
...
| W8 | `references/runtime-python-baseline.txt` missing | the dependency check (#14) is skipped — regenerate the baseline so undeclared imports are caught |
